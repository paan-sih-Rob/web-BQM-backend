...
function tanggal_indonesia($parameter)
{
    ...
    // Periksa panjang array sebelum mengakses elemennya
    if (count($split2) >= 3) {
...
function post_penulis($username)
{
    ...
    // Periksa apakah $data tidak null dan memiliki kunci 'nama_lengkap'
    if ($data !== null && isset($data['nama_lengkap'])) {
...
function set_post_link($post_id)
{
    ...
    if ($data !== null && isset($data['post_type'], $data['post_title_seo'])) {
        $type = $data['post_type'];
        $seo = $data['post_title_seo'];
        return site_url($type . "/" . $seo);
    } else {
        // Handle the case where post data is not found
        return site_url(); // or return an error page URL, for example
    }
}
